...
#define _FP_FRAC_SNANP_SEMIRAW(fs, X)                                 \
  ((_FP_QNANNEGATEDP) ? (_FP_FRAC_HIGH_##fs(X) & _FP_QNANBIT_SH_##fs) \
    ...
    switch (X##_e) {                                                         \
      ...
      case 0:                                                                \
        if (_FP_FRAC_ZEROP_##wc(X))                                          \
          ...
          _FP_FRAC_CLZ_##wc(_FP_UNPACK_CANONICAL_shift, X);                  \
          _FP_UNPACK_CANONICAL_shift -= _FP_FRACXBITS_##fs;                  \
          _FP_FRAC_SLL_##wc(X, (_FP_UNPACK_CANONICAL_shift + _FP_WORKBITS)); \
          X##_e -= (_FP_EXPBIAS_##fs - 1 + _FP_UNPACK_CANONICAL_shift);      \
          X##_c = FP_CLS_NORMAL;                                             \
          FP_SET_EXCEPTION(FP_EX_DENORM);                                    \
        }                                                                    \
        break;                                                               \
...
/* Finish unpacking an fp value in semi-raw mode: the mantissa is
   ...
   other classification is not checked.  */
...
#define _FP_CHECK_FLUSH_ZERO(fs, wc, X)                            \
  do {                                                             \
    if (FP_DENORM_ZERO && X##_e == 0 && !_FP_FRAC_ZEROP_##wc(X)) { \
      _FP_FRAC_SET_##wc(X, _FP_ZEROFRAC_##wc);                     \
      FP_SET_EXCEPTION(FP_EX_DENORM);                              \
    }                                                              \
  } while (0)
...
#define _FP_PACK_SEMIRAW(fs, wc, X)                                       \
  do {                                                                    \
    int _FP_PACK_SEMIRAW_is_tiny = X##_e == 0 && !_FP_FRAC_ZEROP_##wc(X); \
    if (_FP_TININESS_AFTER_ROUNDING && _FP_PACK_SEMIRAW_is_tiny) {        \
    ...
    if (_FP_FRAC_HIGH_##fs(X) & (_FP_OVERFLOW_##fs >> 1)) {               \
      ...
      X##_e++;                                                            \
      if (X##_e == _FP_EXPMAX_##fs) _FP_OVERFLOW_SEMIRAW(fs, wc, X);      \
    }                                                                     \
    _FP_FRAC_SRL_##wc(X, _FP_WORKBITS);                                   \
    if (X##_e == _FP_EXPMAX_##fs && !_FP_FRAC_ZEROP_##wc(X)) {            \
      if (!_FP_KEEPNANFRACP) {                                            \
...
#define _FP_ADD_INTERNAL(fs, wc, R, X, Y, OP)                               \
  do {                                                                      \
    ...
    if (X##_s == Y##_s) {                                                   \
      /* Addition.  */                                                      \
      __label__ add1, add2, add3, add_checked;                              \
      R##_s = X##_s;                                                        \
      ...
      add1:                                                                 \
        ...
        if (_FP_ADD_INTERNAL_ediff <= _FP_WFRACBITS_##fs)                   \
          _FP_FRAC_SRS_##wc(Y, _FP_ADD_INTERNAL_ediff, _FP_WFRACBITS_##fs); \
      ...
      add2:                                                                 \
        ...
        if (_FP_ADD_INTERNAL_ediff <= _FP_WFRACBITS_##fs)                   \
          _FP_FRAC_SRS_##wc(X, _FP_ADD_INTERNAL_ediff, _FP_WFRACBITS_##fs); \
        ...
        if (!_FP_EXP_NORMAL(fs, wc, X)) {                                   \
          if (X##_e == 0) {                                                 \
            ...
            R##_e = 0;                                                      \
            if (_FP_FRAC_ZEROP_##wc(X)) {                                   \
              if (!_FP_FRAC_ZEROP_##wc(Y)) FP_SET_EXCEPTION(FP_EX_DENORM);  \
              _FP_FRAC_COPY_##wc(R, Y);                                     \
              goto add_checked;                                             \
            } else if (_FP_FRAC_ZEROP_##wc(Y)) {                            \
              FP_SET_EXCEPTION(FP_EX_DENORM);                               \
    ...
    add_checked:;                                                           \
      ...
      /* Subtraction.  */                                                   \
      __label__ sub1, sub2, sub3, norm, sub_checked;                        \
      int _FP_ADD_INTERNAL_ediff = X##_e - Y##_e;                           \
      if (_FP_ADD_INTERNAL_ediff > 0) {                                     \
        R##_e = X##_e;                                                      \
        R##_s = X##_s;                                                      \
    ...
    sub3:                                                                   \
      ...
      norm:                                                                 \
        ...
        if (R##_e <= _FP_ADD_INTERNAL_diff) {                               \
          /* R is denormalized.  */                                         \
          _FP_ADD_INTERNAL_diff = _FP_ADD_INTERNAL_diff - R##_e + 1;        \
          _FP_FRAC_SRS_##wc(R, _FP_ADD_INTERNAL_diff, _FP_WFRACBITS_##fs);  \
          R##_e = 0;                                                        \
        } else {                                                            \
      ...
      case _FP_CLS_COMBINE(FP_CLS_NORMAL, FP_CLS_NORMAL):                     \
        switch (Z##_c) {                                                      \
          ...
          case FP_CLS_NORMAL:;                                                \
            ...
            R##_e = _FP_FMA_T##_e;                                            \
            int _FP_FMA_tsh = _FP_FRAC_HIGHBIT_DW_##dwc(fs, _FP_FMA_TD) == 0; \
            ...
            if (_FP_FMA_ediff >= 0) {                                         \
              ...
              if (_FP_FMA_T##_s == Z##_s)                                     \
                _FP_FRAC_ADD_##dwc(_FP_FMA_RD, _FP_FMA_TD, _FP_FMA_ZD);       \
              else {                                                          \
                _FP_FRAC_SUB_##dwc(_FP_FMA_RD, _FP_FMA_TD, _FP_FMA_ZD);       \
                if (_FP_FRAC_NEGP_##dwc(_FP_FMA_RD)) {                        \
                  R##_s = Z##_s;                                              \
                  _FP_FRAC_SUB_##dwc(_FP_FMA_RD, _FP_FMA_ZD, _FP_FMA_TD);     \
                }                                                             \
              }                                                               \
            } else {                                                          \
              R##_e = Z##_e;                                                  \
              ...
              if (Z##_s == _FP_FMA_T##_s)                                     \
                _FP_FRAC_ADD_##dwc(_FP_FMA_RD, _FP_FMA_ZD, _FP_FMA_TD);       \
              else                                                            \
                _FP_FRAC_SUB_##dwc(_FP_FMA_RD, _FP_FMA_ZD, _FP_FMA_TD);       \
            }                                                                 \
  ...
  do {                                                                      \
    ...
    switch (_FP_CLS_COMBINE(X##_c, Y##_c)) {                                \
      ...
      case _FP_CLS_COMBINE(FP_CLS_ZERO, FP_CLS_ZERO):                       \
        ...
        FP_SET_EXCEPTION(                                                   \
            FP_EX_INVALID |                                                 \
            (X##_c == FP_CLS_INF ? FP_EX_INVALID_IDI : FP_EX_INVALID_ZDZ)); \
        break;                                                              \
                                                                            \
      default:                                                              \
        _FP_UNREACHABLE;                                                    \
    ...
    if (ex) {                                                              \
      ...
      if (FP_EX_INVALID_SNAN || FP_EX_INVALID_VC) {                        \
        /* Check separately for each case of "invalid"                     \
           exceptions.  */                                                 \
        if ((ex) == 2) FP_SET_EXCEPTION(FP_EX_INVALID | FP_EX_INVALID_VC); \
        if (_FP_ISSIGNAN(fs, wc, X) || _FP_ISSIGNAN(fs, wc, Y))            \
          FP_SET_EXCEPTION(FP_EX_INVALID | FP_EX_INVALID_SNAN);            \
      } /* Otherwise, we only need to check whether to raise an            \
           exception, not which case or cases it is.  */                   \
      else if ((ex) == 2 || _FP_ISSIGNAN(fs, wc, X) ||                     \
...
/* Helper for comparisons.  If denormal operands would raise an
   ...
   the result, which is checked later with _FP_CMP_CHECK_FLUSH_ZERO).  */
  ...
  do {                                                                \
    ...
    if ((X##_e == _FP_EXPMAX_##fs && !_FP_FRAC_ZEROP_##wc(X)) ||      \
      ...
      _FP_CMP_CHECK_FLUSH_ZERO(fs, wc, X, Y);                         \
                                                                      \
      _FP_CMP_is_zero_x = (!X##_e && _FP_FRAC_ZEROP_##wc(X)) ? 1 : 0; \
      _FP_CMP_is_zero_y = (!Y##_e && _FP_FRAC_ZEROP_##wc(Y)) ? 1 : 0; \
                                                                      \
...
#define _FP_CMP_EQ(fs, wc, ret, X, Y, ex)                                \
  do {                                                                   \
    ...
    if ((X##_e == _FP_EXPMAX_##fs && !_FP_FRAC_ZEROP_##wc(X)) ||         \
      ...
      _FP_CMP_CHECK_FLUSH_ZERO(fs, wc, X, Y);                            \
                                                                         \
      (ret) = !(X##_e == Y##_e && _FP_FRAC_EQ_##wc(X, Y) &&              \
...
#define _FP_CMP_UNORD(fs, wc, ret, X, Y, ex)                          \
  do {                                                                \
    ...
    (ret) = ((X##_e == _FP_EXPMAX_##fs && !_FP_FRAC_ZEROP_##wc(X)) || \
             (Y##_e == _FP_EXPMAX_##fs && !_FP_FRAC_ZEROP_##wc(Y)));  \
    if (ret) _FP_CMP_CHECK_NAN(fs, wc, X, Y, (ex));                   \
  } while (0)
  ...
  do {                                                                \
    ...
    switch (X##_c) {                                                  \
      ...
      case FP_CLS_NORMAL:                                             \
        ...
        R##_c = FP_CLS_NORMAL;                                        \
        if (X##_e & 1) _FP_FRAC_SLL_##wc(X, 1);                       \
        R##_e = X##_e >> 1;                                           \
        _FP_FRAC_SET_##wc(_FP_SQRT_S, _FP_ZEROFRAC_##wc);             \
        _FP_FRAC_SET_##wc(R, _FP_ZEROFRAC_##wc);                      \
        _FP_SQRT_q = _FP_OVERFLOW_##fs >> 1;                          \
        _FP_SQRT_MEAT_##wc(R, _FP_SQRT_S, _FP_SQRT_T, X, _FP_SQRT_q); \
    }                                                                 \
  } while (0)
...
#define _FP_TO_INT(fs, wc, r, X, rsize, rsigned)                               \
  do {                                                                         \
    if (X##_e < _FP_EXPBIAS_##fs) {                                            \
      ...
      if (X##_e == 0) {                                                        \
        if (!_FP_FRAC_ZEROP_##wc(X)) {                                         \
          if (!FP_DENORM_ZERO) FP_SET_EXCEPTION(FP_EX_INEXACT);                \
          FP_SET_EXCEPTION(FP_EX_DENORM);                                      \
        }                                                                      \
      } else                                                                   \
        FP_SET_EXCEPTION(FP_EX_INEXACT);                                       \
      ...
      FP_SET_EXCEPTION(FP_EX_INVALID | FP_EX_INVALID_CVI |                     \
                       ((FP_EX_INVALID_SNAN && _FP_ISSIGNAN(fs, wc, X))        \
                            ? FP_EX_INVALID_SNAN                               \
                            : 0));                                             \
    } else if ((rsigned) != 2 &&                                               \
      ...
      if (rsigned) {                                                           \
        ...
        if (!X##_s) (r) = ~(r);                                                \
                                                                               ...
                                                                               \
      if (_FP_EXPBIAS_##fs + (rsize)-1 < _FP_EXPMAX_##fs && (rsigned) &&       \
          X##_s && X##_e == _FP_EXPBIAS_##fs + (rsize)-1) {                    \
        /* Possibly converting to most negative integer; check the             \
           mantissa.  */                                                       \
        int _FP_TO_INT_inexact = 0;                                            \
        (void)((_FP_FRACBITS_##fs > (rsize)) ? ({                              \
          _FP_FRAC_SRST_##wc(X, _FP_TO_INT_inexact,                            \
                             _FP_FRACBITS_##fs - (rsize), _FP_FRACBITS_##fs);  \
          0;                                                                   \
        })                                                                     \
                                             : 0);                             \
        if (!_FP_FRAC_ZEROP_##wc(X))                                           \
      ...
      if (X##_e >= _FP_EXPBIAS_##fs + _FP_FRACBITS_##fs - 1) {                 \
        ...
        _FP_FRAC_SRST_##wc(X, _FP_TO_INT_inexact,                              \
                           (_FP_FRACBITS_##fs + _FP_EXPBIAS_##fs - 1 - X##_e), \
                           _FP_FRACBITS_##fs);                                 \
        _FP_FRAC_ASSEMBLE_##wc((r), X, (rsize));                               \
      }                                                                        \
      if ((rsigned) && X##_s) (r) = -(r);                                      \
      if ((rsigned) == 2 && X##_e >= _FP_EXPBIAS_##fs + (rsize)-1) {           \
        /* Overflow or converting to the most negative integer.  */            \
        if (X##_e > _FP_EXPBIAS_##fs + (rsize)-1 || !X##_s ||                  \
            (r) != (((typeof(r))1) << ((rsize)-1))) {                          \
          _FP_TO_INT_inexact = 0;                                              \
          FP_SET_EXCEPTION(FP_EX_INVALID | FP_EX_INVALID_CVI);                 \
        }                                                                      \
      }                                                                        \
      if (_FP_TO_INT_inexact) FP_SET_EXCEPTION(FP_EX_INEXACT);                 \
    }                                                                          \
  } while (0)
...
#define _FP_TO_INT_ROUND(fs, wc, r, X, rsize, rsigned)                         \
  do {                                                                         \
    __label__ _FP_TO_INT_ROUND_checked;                                        \
    if (X##_e < _FP_EXPBIAS_##fs) {                                            \
      int _FP_TO_INT_ROUND_rounds_away = 0;                                    \
      if (X##_e == 0) {                                                        \
        if (_FP_FRAC_ZEROP_##wc(X)) {                                          \
          (r) = 0;                                                             \
          goto _FP_TO_INT_ROUND_checked;                                       \
        } else {                                                               \
          FP_SET_EXCEPTION(FP_EX_DENORM);                                      \
          if (FP_DENORM_ZERO) {                                                \
            (r) = 0;                                                           \
      ...
      switch (FP_ROUNDMODE) {                                                  \
        case FP_RND_NEAREST:                                                   \
          _FP_TO_INT_ROUND_rounds_away =                                       \
              (X##_e == _FP_EXPBIAS_##fs - 1 && !_FP_FRAC_ZEROP_##wc(X));      \
          break;                                                               \
        case FP_RND_ZERO:                                                      \
      ...
      if ((rsigned) == 0 && _FP_TO_INT_ROUND_rounds_away && X##_s) {           \
        ...
        (r) = (_FP_TO_INT_ROUND_rounds_away ? (X##_s ? -1 : 1) : 0);           \
        FP_SET_EXCEPTION(FP_EX_INEXACT);                                       \
      }                                                                        \
    } else if ((rsigned) == 2 &&                                               \
               (X##_e >=                                                       \
      ...
      if (X##_e >= _FP_EXPBIAS_##fs + _FP_FRACBITS_##fs - 1) {                 \
        ...
        int _FP_TO_INT_ROUND_rshift =                                          \
            (_FP_FRACBITS_##fs + _FP_EXPBIAS_##fs - 1 - _FP_WORKBITS - X##_e); \
        if (_FP_TO_INT_ROUND_rshift > 0)                                       \
          _FP_FRAC_SRS_##wc(X, _FP_TO_INT_ROUND_rshift, _FP_WFRACBITS_##fs);   \
      ...
      if (X##_e >=                                                             \
          (_FP_EXPBIAS_##fs + (rsize)-1 - ((rsigned) > 0 && !X##_s))) {        \
        if (X##_e > _FP_EXPBIAS_##fs + (rsize)-1 ||                            \
            (X##_e == _FP_EXPBIAS_##fs + (rsize)-1 &&                          \
             (X##_s ? (r) != (((typeof(r))1) << ((rsize)-1))                   \
                    : ((rsigned) > 0 || (r) == 0))) ||                         \
            ((rsigned) > 0 && !X##_s &&                                        \
      ...
      if (_FP_TO_INT_ROUND_inexact) FP_SET_EXCEPTION(FP_EX_INEXACT);           \
    }                                                                          \
  _FP_TO_INT_ROUND_checked:;                                                   \
  } while (0)
  ...
  do {                                                                       \
    ...
    if (r) {                                                                 \
      rtype _FP_FROM_INT_ur = (r);                                           \
                                                                             \
      if ((X##_s = ((r) < 0))) _FP_FROM_INT_ur = -_FP_FROM_INT_ur;           \
                                                                             \
      _FP_STATIC_ASSERT((rsize) <= 2 * _FP_W_TYPE_SIZE, "rsize too large");  \
      (void)(((rsize) <= _FP_W_TYPE_SIZE)                                    \
                 ? ({                                                        \
                     int _FP_FROM_INT_lz;                                    \
                     __FP_CLZ(_FP_FROM_INT_lz, (_FP_W_TYPE)_FP_FROM_INT_ur); \
      ...
      if ((rsize) <= _FP_FRACBITS_##fs ||                                    \
        ...
        if (_FP_EXPBIAS_##fs + _FP_FRACBITS_##fs - 1 - X##_e > 0)            \
          _FP_FRAC_SLL_##wc(                                                 \
              X, (_FP_EXPBIAS_##fs + _FP_FRACBITS_##fs - 1 - X##_e));        \
      } else {                                                               \
        ...
        if (_FP_EXPBIAS_##fs + _FP_WFRACBITS_##fs - 1 < X##_e)               \
          _FP_FROM_INT_ur =                                                  \
              ((_FP_FROM_INT_ur >>                                           \
                (X##_e - _FP_EXPBIAS_##fs - _FP_WFRACBITS_##fs + 1)) |       \
               ((_FP_FROM_INT_ur << ((rsize) - (X##_e - _FP_EXPBIAS_##fs -   \
                                                _FP_WFRACBITS_##fs + 1))) != \
                0));                                                         \
        _FP_FRAC_DISASSEMBLE_##wc(X, _FP_FROM_INT_ur, (rsize));              \
        if ((_FP_EXPBIAS_##fs + _FP_WFRACBITS_##fs - 1 - X##_e) > 0)         \
          _FP_FRAC_SLL_##wc(                                                 \
              X, (_FP_EXPBIAS_##fs + _FP_WFRACBITS_##fs - 1 - X##_e));       \
    ...
    if (_FP_EXP_NORMAL(sfs, swc, S)) {                                      \
      ...
      if (S##_e == 0) {                                                     \
        ...
        if (_FP_FRAC_ZEROP_##swc(S))                                        \
          ...
          _FP_FRAC_SLL_##dwc(                                               \
              D, FP_EXTEND_lz + _FP_FRACBITS_##dfs - _FP_FRACTBITS_##sfs);  \
        ...
        if (!_FP_FRAC_ZEROP_##swc(S)) {                                     \
          if (check_nan && _FP_FRAC_SNANP(sfs, S))                          \
            FP_SET_EXCEPTION(FP_EX_INVALID | FP_EX_INVALID_SNAN);           \
          _FP_FRAC_SLL_##dwc(D, (_FP_FRACBITS_##dfs - _FP_FRACBITS_##sfs)); \
          if (check_nan) _FP_SETQNAN(dfs, dwc, D);                          \
        }                                                                   \
      }                                                                     \
    }                                                                       \
  } while (0)
      ...
      if (D##_e >= _FP_EXPMAX_##dfs)                                           \
        ...
        if (D##_e <= 0) {                                                      \
          if (D##_e < 1 - _FP_FRACBITS_##dfs) {                                \
            ...
            _FP_FRAC_SRS_##swc(                                                \
                S, (_FP_WFRACBITS_##sfs - _FP_WFRACBITS_##dfs + 1 - D##_e),    \
                _FP_WFRACBITS_##sfs);                                          \
          }                                                                    \
          D##_e = 0;                                                           \
        } else                                                                 \
          _FP_FRAC_SRS_##swc(S, (_FP_WFRACBITS_##sfs - _FP_WFRACBITS_##dfs),   \
                             _FP_WFRACBITS_##sfs);                             \
        _FP_FRAC_COPY_##dwc##_##swc(D, S);                                     \
      }                                                                        \
    } else {                                                                   \
...
#define __FP_CLZ(r, x)                                                      \
  do {                                                                      \
    _FP_STATIC_ASSERT((sizeof(_FP_W_TYPE) == sizeof(unsigned int) ||        \
                       (sizeof(_FP_W_TYPE) == sizeof(unsigned long)) ||     \
                       (sizeof(_FP_W_TYPE) == sizeof(unsigned long long))), \
...
#define _FP_DIV_MEAT_N_loop(fs, wc, R, X, Y)                             \
  do {                                                                   \
    ...
    if (_FP_FRAC_GE_1(_FP_DIV_MEAT_N_loop_u, _FP_DIV_MEAT_N_loop_v)) {   \
      _FP_FRAC_SUB_##wc(_FP_DIV_MEAT_N_loop_u, _FP_DIV_MEAT_N_loop_u,    \
    ...
    do {                                                                 \
      ...
      if (_FP_DIV_MEAT_N_loop_msb ||                                     \
          _FP_FRAC_GE_1(_FP_DIV_MEAT_N_loop_u, _FP_DIV_MEAT_N_loop_v)) { \
        _FP_FRAC_SUB_##wc(_FP_DIV_MEAT_N_loop_u, _FP_DIV_MEAT_N_loop_u,  \
                          _FP_DIV_MEAT_N_loop_v);                        \
        _FP_FRAC_LOW_##wc(R) |= 1;                                       \
      }                                                                  \
    ...
    /* If there's anything left in _FP_DIV_MEAT_N_LOOP_U, the result     \
       is inexact.  */                                                   \
    _FP_FRAC_LOW_##wc(R) |= !_FP_FRAC_ZEROP_##wc(_FP_DIV_MEAT_N_loop_u); \
  } while (0)
